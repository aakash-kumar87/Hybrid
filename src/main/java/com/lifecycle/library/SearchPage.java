...
public class SearchPage extends ActionDriver {
	...
	public SearchPage(WebDriver incomingDriver) {
	...
	static List<String> valid = Arrays.asList("Program", "Specialisation", "Course");
	static List<String> studyLevel = Arrays.asList("Undergraduate", "Postgraduate");

	public boolean assertSearch(String searchText) {
		try {
			if (click(Locators.HomePage.SEARCH, "search area")) {
				if (enter(Locators.HomePage.SEARCH, searchText, "Search button in home page")) {
					if (click(Locators.HomePage.SEARCHBUTTON, "Search Button"))
						assertTitle("UNSW Handbook");
					else
	...
	public void verifySearchPageBySearchButton(String searchText) {
		try {
			if (assertSearch(searchText)) {
	...
	public void searchBySeeAllResults(String text) {
		...
		try {
			...
			if (!flag) {
				Reporting.logResults(driver, "fail", "Search by all results  click action",
						"Search by all results was failed");
			}
			Thread.sleep(3000);
	...
	public boolean verifyProgram() {
		...
		try {
			...
			do {
				...
				By xpath = By.xpath("//span[text()='chevron_right']");
				click(xpath, "");
	...
	/**
	 * This Method will Verify take input as Program/Specialsiation/Course and
	 * verify the result list matches Program/Specialsiation/Course and study level
	 * 
	 * @param name
	 */
	public void verifySearch(String name) {
		...
		try {
			...
			String aResult = driver.findElement(Locators.AcamedicItemPage.STUDYAS).getText();

			String aStudyLevel = driver.findElement(Locators.AcamedicItemPage.STUDYLEVEL).getText();
			if (aResult.equalsIgnoreCase(name)) {
				...
				Reporting.logResults(driver, "Fail", "Verify " + name + "in Landing page",
						"Landing page not is a " + name);
			}
			if (aStudyLevel.equalsIgnoreCase(uG)) {
				Reporting.logResults(driver, "pass", "Verify under graduate in landing page",
						"Study level is mentioned as Undergraduate in " + name);
			} else {
				Reporting.logResults(driver, "Fail", "Verify under graduate in landing page",
						"Study level is not mentioned as Undergraduate in " + name);
			}	

			String bResult = driver.findElement(Locators.AcamedicItemPage.STUDYAS).getText();
			String bStudyLevel = driver.findElement(Locators.AcamedicItemPage.STUDYLEVEL).getText();
	...
	public void searchFilters(String name) {
		...
		driver.findElement(By.xpath("//button[text()='" + name + "']")).click();
		try {
			verifyProgramFilters("Study as", "Undergraduate");
			verifyProgramFilters("Faculty", "Undergraduate");
			verifyProgramFilters("Faculty", "Postgraduate");
			verifyProgramFilters("Faculty", "Research");
	 ...
	 * @param filterTypeCheckBox Input as program type or faculty sub programx
	 * @throws InterruptedException
	 * @throws HeadlessException
	 * @throws IOException
	 * @throws AWTException
	 */
	public void verifyProgramFilters(String filter, String studylevel)
		...
		for (int k = 1; k <= sSubItem.size(); k++) {
			...
			if (singleprogram.equalsIgnoreCase("Single Program")) {
				...
				if (flag) {
					...
					if (bResult.equalsIgnoreCase(program)) {
						Reporting.logResults(driver, "Pass", "Verify " + program + " in the landing page",
								"Verified " + program + " in landing page  when " + singleprogram + " is clicked ");
					} else {

						Reporting.logResults(driver, "Fail", "Verify " + program + "in the landing page",
								"Failed to verify " + program + " in landing page  when " + singleprogram
										+ " is clicked ");
						continue;
					}
					ActionDriver action = new ActionDriver(driver);
						...
						bResult = driver.findElement(Locators.AcamedicItemPage.STUDYLEVEL).getText();

						if (bResult.equalsIgnoreCase(studylevel)) {
							Reporting.logResults(driver, "Pass", "Verify " + studylevel + " in the landing page",
									"Verified " + studylevel + " in landing page  when " + singleprogram + " is clicked ");
						} else {
							Reporting.logResults(driver, "Fail", "Verify " + studylevel + "in the landing page",
									"Failed to verify " + studylevel + " in landing page  when " + singleprogram
											+ " is clicked ");
						}
